    ...
    prop @(TestJWKSet -> TestJWK -> Expectation) "with test JWK set" $
      \(TestJWKSet (JWKSet otherJWKs)) (TestJWK testJWK) -> do
        ...
        withJWKCacheFrom (pureJWKCache jwkSet) $ \jwkCache -> do
          Right inputJWT :: Either (AuthError JWTError) SignedJWT <- makeSignedTestJWT testJWK
          eVerifiedClaims :: Either (AuthError JWTError) ClaimsSet <-
            runJOSELogging $ verifyTokenClaims jwkCache (BS.toStrict $ encodeCompact inputJWT)
          Right extractedClaims :: Either (AuthError JWTError) ClaimsSet <- runJOSE $ unsafeGetJWTClaimsSet inputJWT
